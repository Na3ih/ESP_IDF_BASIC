...
const static char http_string[] = "<meta content=\"width=device-width,initial-scale=1\"name=viewport><style>div{width:230px;height:300px;position:absolute;top:0;bottom:0;left:0;right:0;margin:auto}</style><div><h1 align=center>Yeah, bitch!</h1><a href=on.html><img src=on.png></a></div>";
	...
	if (err == ERR_OK) {
		...
		if(first_line) {
			...
			if(strstr(first_line, "GET / ")) {
                ...
                netconn_write(conn, http_string, sizeof(http_string) - 1, NETCONN_NOCOPY);					
			}	
